...
def update_cache():
	...
	update_meta_data(db, modified_files)
...
def get_meta_sql_from_dict(dict):
...
def update_meta_data(db, modified_files):
	sql_insert = None
	insert_data = []
	for file in modified_files:
		meta = get_exif_info(file)
		if sql_insert == None:
			sql_insert = get_meta_sql_from_dict(meta)
...
# If being executed (instead of imported), call refresh_db() automatically
